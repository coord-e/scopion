...
TEST_F(parserTest, funcVal) {
  EXPECT_EQ(parser::parse("(){(){1;};}").ast,
            ast::expr(ast::function({}, std::vector<ast::expr>{ast::function(
                                            {}, {ast::integer(1)})})));
}
...
TEST_F(parserTest, arrayVal) {
  EXPECT_EQ(
      ...
      ast::expr(ast::function({}, {ast::array({ast::integer(1), ast::integer(2),
                                               ast::integer(3)})})));
} // namespace
  ...
  EXPECT_EQ(parser::parse("(){a=1;}").ast,
            ast::expr(ast::function(
                {}, {ast::binary_op<ast::assign>(
                        ast::variable("a", true, false), ast::integer(1))})));
}
...
TEST_F(parserTest, callOp) {
  EXPECT_EQ(
      ...
      ast::expr(ast::function(
          {}, {ast::binary_op<ast::call>(ast::variable("a", false, true),
...
TEST_F(parserTest, atOp) {
  EXPECT_EQ(parser::parse("(){a[1];}").ast,
            ast::expr(ast::function(
                {}, {ast::binary_op<ast::at>(ast::variable("a", false, false),
                                             ast::integer(1))})));
}
