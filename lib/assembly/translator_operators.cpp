...
namespace assembly
{
value* translator::apply_op(ast::binary_op<ast::add> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateAdd(args[0]->getLLVM(), args[1]->getLLVM()),
                   op);
}

value* translator::apply_op(ast::binary_op<ast::sub> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateSub(args[0]->getLLVM(), args[1]->getLLVM()),
                   op);
}

value* translator::apply_op(ast::binary_op<ast::mul> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateMul(args[0]->getLLVM(), args[1]->getLLVM()),
                   op);
}

value* translator::apply_op(ast::binary_op<ast::div> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateSDiv(args[0]->getLLVM(), args[1]->getLLVM()),
                   op);
}

value* translator::apply_op(ast::binary_op<ast::rem> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateSRem(args[0]->getLLVM(), args[1]->getLLVM()),
                   op);
}

value* translator::apply_op(ast::binary_op<ast::shl> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateShl(args[0]->getLLVM(), args[1]->getLLVM()),
                   op);
}

value* translator::apply_op(ast::binary_op<ast::shr> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateLShr(args[0]->getLLVM(), args[1]->getLLVM()),
                   op);
}

value* translator::apply_op(ast::binary_op<ast::iand> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateAnd(args[0]->getLLVM(), args[1]->getLLVM()),
                   op);
}

value* translator::apply_op(ast::binary_op<ast::ior> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateOr(args[0]->getLLVM(), args[1]->getLLVM()),
                   op);
}

value* translator::apply_op(ast::binary_op<ast::ixor> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateXor(args[0]->getLLVM(), args[1]->getLLVM()),
                   op);
}

value* translator::apply_op(ast::binary_op<ast::land> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateAnd(
...
value* translator::apply_op(ast::binary_op<ast::lor> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateOr(
                       builder_.CreateICmpNE(
                           args[0]->getLLVM(),
                           llvm::Constant::getNullValue(builder_.getInt1Ty())),
                       builder_.CreateICmpNE(
...
value* translator::apply_op(ast::binary_op<ast::eeq> const& op,
                            std::vector<value*> const& args)
{
  return new value(
      builder_.CreateICmpEQ(args[0]->getLLVM(), args[1]->getLLVM()), op);
}

value* translator::apply_op(ast::binary_op<ast::neq> const& op,
                            std::vector<value*> const& args)
{
  return new value(
      builder_.CreateICmpNE(args[0]->getLLVM(), args[1]->getLLVM()), op);
}

value* translator::apply_op(ast::binary_op<ast::gt> const& op,
                            std::vector<value*> const& args)
{
  return new value(
      builder_.CreateICmpSGT(args[0]->getLLVM(), args[1]->getLLVM()), op);
}

value* translator::apply_op(ast::binary_op<ast::lt> const& op,
                            std::vector<value*> const& args)
{
  return new value(
      builder_.CreateICmpSLT(args[0]->getLLVM(), args[1]->getLLVM()), op);
}

value* translator::apply_op(ast::binary_op<ast::gtq> const& op,
                            std::vector<value*> const& args)
{
  return new value(
      builder_.CreateICmpSGE(args[0]->getLLVM(), args[1]->getLLVM()), op);
}

value* translator::apply_op(ast::binary_op<ast::ltq> const& op,
                            std::vector<value*> const& args)
{
  return new value(
      builder_.CreateICmpSLE(args[0]->getLLVM(), args[1]->getLLVM()), op);
}

value* translator::apply_op(ast::binary_op<ast::assign> const& op,
                            std::vector<value*> const& args)
{
  ...
  if (!lval) {  // first appear in the block (variable declaration)
    ...
    if (!parent) {  // not a member of array or structure
      ...
      if (!args[1]->isLazy())
        args[1]->setLLVM(lval = builder_.CreateAlloca(rval->getType(), nullptr,
                                                      ast::val(lvar)));
      ...
      if (parent->getLLVM()->getType()->isStructTy()) {  // structure
        ...
        throw error("Variadic structure: This feature is not supported yet",
        ...
        // lval, and add args[1] to fields
        throw error("Variadic array: This feature is not supported yet",
                    ast::attr(op).where, code_range_);
      } else {
    ...
    if (args[1]->isLazy()) {
      if (auto parent = args[0]->getParent()) {
        ...
        parent
            ->fields()[ast::val(ast::unpack<ast::identifier>(ast::val(
                ast::unpack<ast::op<ast::dot, 2>>(ast::val(op)[0]))[1]))]
            .second = args[1];
      } else {
        thisScope_
            ->symbols()[ast::val(ast::unpack<ast::variable>(ast::val(op)[0]))] =
            args[1];
      }
    }
  }
...
value* translator::apply_op(ast::binary_op<ast::call> const& op,
                            std::vector<value*> const& args)
{
  ...
  if (args[0]->getLLVM()->getType()->isLabelTy()) {
    if (!ast::val(ast::unpack<ast::arglist>(ast::val(op)[1])).empty()) {
      throw error("Calling scope with arguments is not allowed",
                  ast::attr(op).where, code_range_);
    } else {
    ...
    return new value(
        builder_.CreateCall(tocall, llvm::ArrayRef<llvm::Value*>(arg_values)),
        op);
  }
}

value* translator::apply_op(ast::binary_op<ast::at> const& op,
                            std::vector<value*> const& args)
{
  ...
  try {
    ...
    if (std::none_of(args[0]->fields().begin(), args[0]->fields().end(),
      ...
      return ast::attr(op).lval ? new value(ep, op)
                                : new value(builder_.CreateLoad(ep), op);
    } else {  // contains lazy
      throw error(
...
value* translator::apply_op(ast::binary_op<ast::dot> const& op,
                            std::vector<value*> const& args)
{
  auto lval = args[0]->getLLVM();
  ...
  if (elm == args[0]->fields().end()) {
    throw error("No member named \"" + id + "\" in the structure",
                ast::attr(op).where, code_range_);
  }
...
value* translator::apply_op(ast::single_op<ast::ret> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateRet(args[0]->getLLVM()), op);
}

value* translator::apply_op(ast::single_op<ast::lnot> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateXor(
                       builder_.CreateICmpNE(
                           args[0]->getLLVM(),
                           llvm::Constant::getNullValue(builder_.getInt1Ty())),
                       builder_.getInt32(1)),
...
value* translator::apply_op(ast::single_op<ast::inot> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateXor(args[0]->getLLVM(), builder_.getInt32(1)),
                   op);
}

value* translator::apply_op(ast::single_op<ast::inc> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateAdd(args[0]->getLLVM(), builder_.getInt32(1)),
                   op);
}

value* translator::apply_op(ast::single_op<ast::dec> const& op,
                            std::vector<value*> const& args)
{
  return new value(builder_.CreateSub(args[0]->getLLVM(), builder_.getInt32(1)),
                   op);
}

value* translator::apply_op(ast::ternary_op<ast::cond> const& op,
                            std::vector<value*> const& args)
{
  if (!args[1]->getLLVM()->getType()->isLabelTy() ||
      !args[2]->getLLVM()->getType()->isLabelTy())
  ...
  try {
    ...
    throw error("Applying non-scope value as scope", ast::attr(op).where,
                code_range_);
  }
  ...
  if (args[0]->isLazy())
    throw error("Conditions with lazy value is not supported",
                ast::attr(op).where, code_range_);
